...
该图描述通用 scanner pipeline，不应与 Cargo detector 的窄取消 seam 混淆。当前 `cleaner_cargo_detect_with_cancel` 的 caller-owned token 只进入同步 scan 之后的 fixed-input/evidence collection；同步 scan 仍创建并拥有独立 token。CLI 尚未把 Ctrl-C、`sweepx cancel` 或 live operation registry 接到这个 caller-owned token。
...
当前实现已经具备 bounded durable event envelope/stream validator、opaque durable cursor 约束与 schema/golden 覆盖；Linux 还接入了 bounded SQLite event journal，在一个事务中写入完整事件流和 terminal snapshot，Core `status` 先查 journal。Linux 已公开 `sweepx --format ndjson status --operation-id ID --watch [--after SXCUR1]` 的 degraded completed-stream replay：先做一次同 snapshot 全量校验，然后对已完成且已持久化的 stream 按每页最多 1024 条事件续读；unknown 但语法有效的 cursor 返回单独的 `stream.reset_required`，malformed cursor/usage 返回 usage error。由于事件目前仍在 scan 完成后批量构造，它不是 live sink，不等待新事件，也不创建后台 operation，尚未满足 live streaming 或 runtime qualification；因此 `scan --format ndjson` 仍必须在 admission 前返回 unsupported，不能输出伪实时事件流。当前 scan 机器输出只开放 bounded JSON。macOS 保留 legacy operation snapshot，不具备 SQLite journal/replay；Windows durable snapshot store 同样失败关闭，直到实现 current-user-private DACL、逐组件 reparse-point 拒绝和私有文件 ACL，默认路径不创建，显式 `--state-dir` 也拒绝。
...
Z0 解析已扫描的 `Cargo.toml` 与 `.cargo/config*`，只把已 admission、out-of-source、完整且结构落入 allowlist 的 target root 提为候选；目录条件必把基础 R1 单文件风险提升为至少 R2。当前实现已具备 Scanner locator batch reader，并在 Linux/macOS/Windows backend 上以有界、handle-relative/handle-bound 的方式读取固定输入；workspace 固定输入路径只读取已 admission 的 `Cargo.toml`、`.cargo/config` 与 `.cargo/config.toml`。`data.hints[].evidence.cargo.configScope` 现公开 `cargo.config-scope.v1` wire contract：顶层字段为 `schema`、`decoderId`、`workspace`、`ancestorConfigs`、`cargoHomeConfig`、`environment`、`cli`、`invocationCwd`、`precedenceComplete`、`blockers[]`。其中 `workspace.pairSnapshot` 使用 `stable_snapshot|not_checked|failed`，`workspace.config/configToml` 的合同枚举是 `present|verified_absent|not_checked|failed`，但生产路径在 pair 非稳定时会把时间点式 absence 降为 `not_checked`；`workspace.selected` 使用 `config|config_toml|none|not_checked`，`workspace.targetDirDeclaration` 使用 `known|verified_absent|not_checked|unknown`。`environment` 只记录 `CARGO_TARGET_DIR`、`CARGO_BUILD_TARGET_DIR`、`CARGO_HOME` 的存在性：`valueRedacted` 始终存在，`present_redacted` 时为 `true`、`verified_absent` 时为 `false`，且不序列化任何变量值；workspace config 中的 `build.target-dir` 也只在 `known` 时投影 `source=config|config_toml` 与 `valueRedacted=true`，不暴露原始相对路径值。只有显式设置且通过绝对路径校验的 `CARGO_HOME` 会在 Cleaner compatibility gate 之后被私下捕获，并在 scan 后对同一 home identity 重验；随后只观察该 home 根下直属 `config` 与 `config.toml` 的存在性。至少命中一个时 `cargoHomeConfig` 仅投影 `present_redacted` 并附加 `cargo_home_config_present_redacted` blocker；该观察是时间点式、non-atomic 的，不声称数量、顺序、选择或稳定 absence。scanner 对命中的直属文件只执行有界、no-follow、handle-bound 的 metadata inspection；配置内容不会被读取、解析、使用或序列化，`target-dir` 值也不会被提取。未命中，以及 `CARGO_HOME` 未显式设置而 Cargo 会采用默认 home 时，都保持 `not_checked(config_scope_not_checked)`，不能投影为 `verified_absent`。私有环境值和 home/config 路径同样不进入 wire evidence。SweepX 的 `cargo-detect` CLI surface 没有 Cargo passthrough `--target-dir` 或 `--config`，因此该入口把 `cli.targetDir` 与 `cli.configOverrides` 投影为结构性 `verified_absent`；普通 Core 调用默认保持 `not_checked`，只有显式采用 no-overrides invocation contract 才能作同样声明。process cwd 也仅在 compatibility gate 通过后私下捕获，并记录 capture-time identity；随后只在精确 native path 与重验后的 workspace root identity 都匹配时投影 `path_matches_revalidated_workspace_root`。由于未跨阶段持有 cwd handle，该状态仍保留 `invocation_cwd_identity_not_bound` blocker；cwd path 本身不进入序列化结果。两份 workspace 配置通过同一个 retained `.cargo` handle 和单个有界枚举 cursor 观察，ASCII 大小写 alias 与重复项失败关闭；但目录枚举不能排除并发 ABA，未观察到的成员仍保持 `not_checked`，不能升级为 `verified_absent` 或稳定选择。当前稳定 blocker 词表包含 `ancestor_configs_*|cargo_home_config_*|cargo_*_present_redacted|cli_*|invocation_cwd_*|workspace_config_*` 与 `workspace_target_dir_invalid`；因此 `precedenceComplete` 当前保持 `false`。Cargo-home presence ledger 不读取、解析、使用或序列化配置内容来闭合 precedence，ancestor configs 与 workspace pair 也仍未解决；`cargo.targetDir` 在常规 detector 输出中继续保持 `NotChecked(config_scope_not_checked)`，依赖它的 `cargo.targetShape` 继续保持 `Unknown(config_scope_not_checked)`。因此当前 `cargo-detect` 结果仍是 hint/report-only，顶层 `candidateAllowed=false`、`planAllowed=false`、`approvalAllowed=false`、`executionAllowed=false`。共享 `CARGO_TARGET_DIR`、活动 cargo/rustc/IDE、发布/签名证据或 unknown component 再升 R3/report-only。mtime 仅弱信号。Z1 `cargo metadata --no-deps --locked --offline` 只能显式 opt-in、固定 executable/argv、可丢 home/target、无网络和写监控；不调用 `cargo clean`。来源：[Cargo metadata](https://doc.rust-lang.org/cargo/commands/cargo-metadata.html)、[Cargo clean](https://doc.rust-lang.org/cargo/commands/cargo-clean.html)（访问：2026-08-26）。

当前 Core 还公开 `cleaner_cargo_detect_with_cancel(context, request, &CancellationToken)`，供库调用方持有并协作取消 **scan 之后** 的 fixed-input/evidence collection。这个 token 不属于前面的同步 filesystem scan：同步 scan 仍使用独立的内部 token，调用方即使在 scan 期间触发自己的 token，也要到 post-scan collection 开始时才会被观察。取消使相应 Cargo typed evidence 以 `unknown(cancelled)` 失败关闭，并把 detector terminal envelope 投影为 `status=cancelled`、exit 10、`reasonCode=cancelled`；它不会把部分证据提升为 candidate，也不会产生 plan、approval、permit、execution 或任何 mutation authority。旧的 `cleaner_cargo_detect` 仍以新建 token 调用该入口。CLI 当前也只创建一个未连接到信号的本地 token，没有 Ctrl-C handler、`sweepx cancel` 接线或 live in-process operation registry；因此这只是 library integration seam，不是现有用户可触发的命令取消能力。
...
2. **M1 有界 Scanner**：portable adapters、aggregate/cache/spill、TUI、bounded JSON、fixtures/oracle；Linux 已完成 bounded SQLite journal、单事务完整流/terminal snapshot，以及对已完成且已持久化 stream 的 degraded `status --watch` completed replay。该 replay 先做一次同 snapshot 全量校验，随后按每页最多 1024 条事件续读；unknown 但语法有效的 cursor 返回 `stream.reset_required`。由于事件仍在 scan 后批量构造，流式接口仍是 non-live、非 runtime-qualified，`scan --format ndjson` 继续 disabled；无 deletion code path。
