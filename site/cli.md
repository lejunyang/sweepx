...
读取日志需要与加速相同的提权卷句柄，因此未提权运行不会记录任何证据，行为与该功能引入之前完全一致。

状态目录与被扫描目录树位于同一个卷时（这是默认情形，状态目录位于 `%LOCALAPPDATA%` 下）同样可以通过
校验。写入缓存本身会被记录到它所记录的那个卷的日志中，因此 SweepX 会读取发生变更的记录，当这些记录
全部属于缓存自身的目录时，即判定该卷处于静止状态。该区间内出现任何其他内容——包括与缓存写入同时发生的、
位于该卷其他位置的写入——都会拒绝复用并上报 `stale_preview`。证据
