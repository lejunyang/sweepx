...
- 因此当前 `precedenceComplete=false`。ancestor/Cargo-home 与 workspace pair 的 precedence source 仍未闭合，`targetDir` 继续是 `not_checked(config_scope_not_checked)`，`targetShape` 继续是 `unknown(config_scope_not_checked)`。
