...
当前 CLI 不请求提权，不调用清理管理器，也不因 read error 自动扩大范围。这里的“只读”针对扫描目标；`scan` 可以在指定 state directory 写自己的状态 snapshot，P3 audit library 也可以写 SweepX 自己的审计状态。它们都不修改被扫描的目标。
