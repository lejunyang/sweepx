...
- run `cache status` and consume `cache.status.result`, treating `absent/available/degraded`, counts, approximate bytes, and health fields as read-only diagnostics only and never as live/current filesystem facts;
...
Current scan automation should use bounded JSON. `scan --format ndjson` remains disabled; Linux exposes only degraded completed replay for `status --watch --format ndjson`: it performs one same-snapshot full validation, then reads pages of at most 1024 events from a completed, persisted stream; an unknown but syntactically valid cursor yields `stream.reset_required`, while malformed cursor usage remains a usage error. `cache status` supports human/JSON only, treats NDJSON as a usage error, inspects only `preview-cache/current.json`, the current generation file, and the flat `generations/` / `quarantine/` directories, and never scans, repairs, quarantines, or reveals cached entries/path contents; missing state/cache returns `absent` without creating directories. Because events are still constructed after the scan, the completed replay is non-live, does not wait for new events, does not create a background operation, does not support cancel, and is still not runtime-qualified. `--locale` changes human-facing text only; it does not change schema keys, status values, reason codes, or capability states. An Agent should preserve those stable fields through translation and summary.
