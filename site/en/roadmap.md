...
| P2 analysis/TUI/Cleaner | Bounded explain, live directory browsing through `scan --tui`, metadata-only Cleaner, and read-only preview-cache diagnostics | Imported explain input is report-only; TUI detail expansion is now single-flight background rescan with a 2 s deadline, late-result discard, and a 32-worker stuck cap; `cache status` performs bounded preview-cache inspection only: missing state returns `absent` without creating directories, `available` means only that bounded cache structure/validation is readable, and warnings/errors/quarantine presence degrade the result; signing/sandbox/full cross-surface qualification open |
