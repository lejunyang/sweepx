...
| P1 scanner CLI | Linux read-only scan is degraded; macOS handle-bound scan is degraded; Windows handle-relative scan is degraded; all are exposed through `scan` / `scan --tui`; Unix status snapshots work | Windows durable state is disabled, `state_dir` defaults to `None`, explicit `--state-dir` fails closed, scan NDJSON and live cancel are disabled, and three-platform/resource gates remain open |
