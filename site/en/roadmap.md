...
| P1 scanner CLI | Linux read-only scan is degraded; macOS handle-bound scan is degraded; Windows handle-relative scan is degraded; all are exposed through `scan` / `scan --tui`; Unix status snapshots work | Windows durable state is disabled, `state_dir` defaults to `None`, explicit `--state-dir` fails closed, live cancel remains disabled, and protocol validators exist while scan NDJSON stays disabled; three-platform/resource gates remain open |
...
| P3 plan/audit/simulation | Immutable plans, simulation-only authorization, Unix audit/recovery, and a sealed fake executor are implemented; durable event envelope/stream validation plus schema/golden coverage now exist | No CLI wiring, trusted HumanApproval broker, native path, real revalidation, SQLite journaling/replay, atomic terminal persistence, or platform adapter; the phase is not qualified |
