...
| P2 analysis/TUI/Cleaner | Bounded explain, progressive directory browsing through `scan --tui`, metadata-only Cleaner, report-only project and platform user-cache `junk` rules, plus read-only preview-cache diagnostics | Imported explain input is report-only; TUI enters after root admission, lists the current level first, then updates direct-directory sizes with throttled 120 ms lower-bound snapshots before converging to exact/incomplete final evidence. Single-flight work uses a 30 s no-progress deadline, late-result discard, one-slot backpressure, and a 32-worker stuck cap. `cache status` remains bounded and read-only; platform rules use narrow roots and remain report-only, while Windows OS handlers, Linux temporary roots, and shared stores are still excluded; signing/sandbox/full cross-surface qualification remain open |
