...
- `precedenceComplete` therefore remains `false`. Ancestor/Cargo-home sources and the workspace pair are still intentionally unresolved, so `targetDir` remains `not_checked(config_scope_not_checked)` and `targetShape` remains `unknown(config_scope_not_checked)`.
