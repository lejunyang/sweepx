...
| P2 analysis/TUI/Cleaner | bounded explain、`scan --tui` 渐进目录浏览、metadata-only Cleaner 与 `junk` report-only 规则可运行，以及 preview cache 只读诊断 | imported explain input report-only；TUI 根准入后立即进入，先列当前层，再以 single-flight 后台 rescan 聚合直接子目录大小，30 s deadline、late result discard、32 stuck-worker cap；`cache status` 只做 bounded preview-cache inspection；Cargo detector 与 `junk` 均保持 report-only；签名/沙箱/完整跨表面资格未闭合 |
