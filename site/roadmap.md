...
| P1 scanner CLI | Linux read-only scan degraded；macOS handle-bound read-only scan degraded；status snapshot 可用 | Windows scanner fail-closed unsupported；live cancel disabled；三平台/资源 gate 未闭合 |
