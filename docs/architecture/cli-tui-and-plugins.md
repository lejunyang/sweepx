...
2. 当前固定输入收集器已能稳定读取 `Cargo.toml`、`.cargo/config` 与 `.cargo/config.toml`，并对替换、symlink/reparse、mount 变化、资源上限和取消 fail closed。workspace 证据在 manifest 合法且绑定成立时可投影为 `Known`。同时 `data.hints[].evidence.cargo.configScope` 公开 `cargo.config-scope.v1`：顶层字段固定为 `schema`、`decoderId`、`workspace`、`ancestorConfigs`、`cargoHomeConfig`、`environment`、`cli`、`invocationCwd`、`precedenceComplete`、`blockers[]`。其中 `workspace.pairSnapshot` 使用 `stable_snapshot|not_checked|failed`，`workspace.config/configToml` 的合同枚举是 `present|verified_absent|not_checked|failed`，但生产路径在 pair 非稳定时会把时间点式 absence 降为 `not_checked`；`workspace.selected` 使用 `config|config_toml|none|not_checked`，`workspace.targetDirDeclaration` 使用 `known|verified_absent|not_checked|unknown`。`environment` 只记录 `CARGO_TARGET_DIR`、`CARGO_BUILD_TARGET_DIR`、`CARGO_HOME` 的存在性；`valueRedacted` 始终存在，`present_redacted` 时为 `true`、`verified_absent` 时为 `false`，且不序列化任何变量值。workspace `build.target-dir` 声明在投影中同样只返回 `source=config|config_toml` 与 `valueRedacted=true`，不回传原始相对路径值。只有显式绝对 `CARGO_HOME` 会在 Cleaner compatibility gate 之后被私下捕获，并在 scan 后重验；collector 只观察该 home 根下直属 `config` / `config.toml` 是否存在。命中只投影 `cargoHomeConfig.state=present_redacted`；scanner 对命中的直属文件只做有界、no-follow、handle-bound metadata inspection，配置内容不会被读取、解析、使用或序列化，`target-dir` 值也不会被提取，环境值和 home/config 路径同样不序列化。未命中以及默认 home（未显式设置 `CARGO_HOME`）保持 `not_checked`，不能提升为 `verified_absent`。SweepX 的 `cargo-detect` CLI surface 没有 Cargo passthrough `--target-dir` 或 `--config`，因此该入口把 `cli.targetDir` 与 `cli.configOverrides` 投影为 `verified_absent`；普通 Core 调用默认保持 `not_checked`，只有显式采用 no-overrides invocation contract 才能作同样声明。
3. process cwd 仅在 Cleaner compatibility gate 通过后私下捕获。它保持 native 且不序列化，并记录 capture-time identity；随后只与重验后的 workspace root 做精确 native path 和 identity 比较。两者匹配仅投影 `path_matches_revalidated_workspace_root`，因为未跨阶段持有 cwd handle，仍保留 `invocation_cwd_identity_not_bound` blocker。精确路径不匹配表示该 layout 未绑定：`invocationCwd.state=not_checked` 且 blocker 为 `invocation_cwd_not_bound`；cwd 捕获、输入校验或 live identity 重验失败则为 `invocationCwd.state=failed` 与 `invocation_cwd_binding_failed`，而不是扩大 authority。
4. `.cargo/config` 与 `.cargo/config.toml` 现在通过同一个 retained `.cargo` handle 和单个有界枚举 cursor 观察；精确命中会在其所在 batch 内完成 no-follow inspect/read，并继续枚举到 EOF，ASCII 大小写 alias 与重复项失败关闭。该观察仍不能排除并发 create/delete/rename 的 ABA，因此不能称为原子 `pairSnapshot`；未观察到的成员投影为 `not_checked`，不能提升为 `verified_absent`。Cargo-home 的直属 config presence 观察同样是 non-atomic，不提供 pair、顺序、选择或 absence 证明；命中时保留 `cargo_home_config_present_redacted` blocker。稳定 blocker 词表现在覆盖 `ancestor_configs_*|cargo_home_config_*|cargo_*_present_redacted|cli_*|invocation_cwd_*|workspace_config_*` 与 `workspace_target_dir_invalid`。
5. Analyze 仍要求 target 目录位于已 admission workspace 或明确配置路径、out-of-source/生成物边界可证明、目录扫描完整、无 symlink/reparse/mount/provider 边界。检查活动 `cargo`/`rustc`/build-script/IDE/rust-analyzer 只产生时间点 blocker；“未观察到”不证明无人使用。显式 Cargo-home ledger 只证明直属 config 的 redacted presence；配置内容不会被读取、解析、使用或序列化，`target-dir` 值也不会被提取，环境值与路径也不公开，因而不能闭合 precedence。ancestor configs 与 workspace pair 仍未解决，sharing/activity 也未闭合。因此 `precedenceComplete=false`，当前 detector 中的 `cargo.targetDir` 继续保持 `NotChecked(config_scope_not_checked)`，依赖它的 `cargo.targetShape` 继续保持 `Unknown(config_scope_not_checked)`，即使同一 workspace 下的 target 目录聚合与名称线索已存在。当前 `cargo-detect` 输出因此继续是 hint/report-only：`candidateAllowed=false`、`planAllowed=false`、`approvalAllowed=false`、`executionAllowed=false`。
6. `cargo.targetShape` 只由 metadata-only shape classifier 产生：它检查已扫描相对名称、对象类型与 Cargo 约定的顶层生成结构，绝不打开或识别文件内容；只要出现不在 allowlist 的顶层组件、special/link、不可读项或分类不确定，就是 `unknown` 并 report-only。它不能证明任意文件“不是源码/签名资产”，因此低风险只适用于结构完全落入发布期 allowlist 的封闭目录；多个 workspace/环境共享 `CARGO_TARGET_DIR`、release/package/signing evidence、config 缺失或目标跨 mount 时提升 R2/R3 或 report-only。mtime 只能是弱 activity signal，不能单独证明未使用。
7. Plan 默认是整个已封闭 target directory 的平台 Trash，展示冷启动/重编译成本和 potentially reclaimable；不调用 `cargo clean`，因为 strict Z0 不启动工具。审批后仍完整重验 descendant manifest；新增文件使 STALE。Trash 失败不 `rm -rf`。
