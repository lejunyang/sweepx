...
| NTFS USN change tokens | `FSCTL_QUERY_USN_JOURNAL` / `FSCTL_READ_USN_JOURNAL` | **Landed**, both control codes. QUERY stores a per-volume token in the generation and re-checks it on load, upgrading `stale_preview` to `verified_preview`; confirmed elevated in both directions on a live volume. READ closes the gap that made this useless by default: because the cache's own write is journalled on the volume it records, a same-volume cache invalidated itself, so the changed range is now read and accepted as quiet only when every record belongs to the cache's own directories. Attribution keys on parent reference numbers read from open handles, never on names, and fails closed on an unresolvable exclusion set or an unreadable range. Still elevation-gated — see the access-mask table in `native-scan-qualification.md`. |
