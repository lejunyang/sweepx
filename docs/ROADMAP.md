...
| `f042e39` | Typed Cargo evidence surfaced to CLI/Core | `cleaner.cargo-detect` now exposes typed Cargo evidence and a dedicated degraded capability entry. When manifest binding is valid, workspace evidence can be `Known`; however, because global `CARGO_TARGET_DIR` override scope is still unresolved, detector output keeps `targetDir` at `NotChecked(config_scope_not_checked)` and `targetShape` at `Unknown(config_scope_not_checked)`. Results remain read-only hint/report-only with `candidateAllowed=false`, `planAllowed=false`, `approvalAllowed=false`, and `executionAllowed=false`. |
