...
- `sweepx scan` performs a synchronous, read-only Linux scan and reports that cell as **degraded**. macOS now has a handle-bound read-only scanner exposed through the same `sweepx scan` and `sweepx scan --tui` entry points, and that cell is also **degraded**. Windows remains fail-closed **unsupported**.
- On Unix, `status` reads a durable terminal snapshot. Windows durable snapshots are disabled until private DACL and reparse-point checks exist. `cancel` exists so it can return an honest disposition, but cancellation is **disabled** because there is no live in-process operation registry.
