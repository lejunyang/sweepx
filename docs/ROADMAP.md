...
- On Unix, `status` reads a durable terminal snapshot. Windows durable state is disabled: `state_dir` defaults to `None`, no terminal snapshot is persisted, and an explicit `--state-dir` fails closed. `cancel` exists so it can return an honest disposition, but cancellation is **disabled** because there is no live in-process operation registry.
