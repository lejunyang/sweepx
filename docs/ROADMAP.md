...
- P3 libraries now model immutable plans, explicitly simulation-only authorization, Unix-only durable audit/recovery state, and a sealed deterministic fake executor. There is no CLI `plan`, `approve`, or `execute` command, the executor accepts no native path, and there is no native filesystem mutation adapter. Durable event validation is implemented, and Linux now has a bounded SQLite journal that persists the complete stream and terminal snapshot in one transaction, with journal-first Core status. The event-journal crate retains a Linux-test-only bounded cursor replay/reset substrate that is not wired into Core or the CLI. Events are still constructed as a batch after scanning; the live sink, runtime qualification, public `status --watch`/NDJSON, and non-Linux journal parity remain incomplete, so scan NDJSON stays disabled. This is still not release-qualified native audit storage and must not be described as native execution qualification.
