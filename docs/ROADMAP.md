...
- The sole `sweepx` binary renders a bounded human table by default. `sweepx scan --tui` consumes the same live typed scan result and provides read-only directory enter/back navigation without an intermediate JSON file. Detail expansion runs as a single-flight background rescan with a 2 s deadline; navigation and quit do not wait for a non-cooperative worker, late completions are discarded, and detached stuck workers are bounded by a process-wide cap of 32.
