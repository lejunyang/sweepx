...
| `f042e39` | Typed Cargo evidence surfaced to CLI/Core | `cleaner.cargo-detect` now exposes typed Cargo evidence and a dedicated degraded capability entry. `data.hints[].evidence.cargo.configScope` uses the redacted `cargo.config-scope.v1` wire contract to report workspace/config source states, precedence blockers, redacted environment-variable presence, and redacted workspace `target-dir` declaration metadata. Because `.cargo/config` and `.cargo/config.toml` are still separate bounded reads, pair selection is not atomic; on case-insensitive platforms that also means the contract cannot prove an alternate-casing variant is absent. `precedenceComplete` therefore remains `false`, the global `CARGO_TARGET_DIR` override scope remains unresolved, detector output keeps `targetDir` at `NotChecked(config_scope_not_checked)` and `targetShape` at `Unknown(config_scope_not_checked)`, and results remain read-only hint/report-only with `candidateAllowed=false`, `planAllowed=false`, `approvalAllowed=false`, and `executionAllowed=false`. |
