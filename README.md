...
| `sweepx cleaner cargo-detect` | **实验性** live-only Cargo target 只读检测入口 | Scanner 现有有界 locator batch reader，并在三平台 backend 上提供 handle-relative/handle-bound 的有界文件读取路径；Cargo 固定输入收集器只读取已 admission 的 `Cargo.toml` 与 `.cargo/config*`。`data.hints[].evidence.cargo.configScope` 现公开 `cargo.config-scope.v1`：它只记录 workspace pair/config 状态、外部来源状态、环境变量存在性和 redacted declaration metadata，不记录环境变量值，也不公开 workspace config `target-dir` 原文。由于 pair snapshot、ancestor/home/CLI/cwd 绑定仍未闭合，`precedenceComplete=false`，`targetDir` 继续保持 `NotChecked(config_scope_not_checked)`，`targetShape` 继续保持 `Unknown(config_scope_not_checked)`；结果继续只产生 hint/report-only，不会产生 candidate、计划、授权或执行 |
